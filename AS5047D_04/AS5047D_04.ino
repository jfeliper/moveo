...
#define STEPPER_ACCEL     100000
#define STEPPER_SPEED     100
#define DESPLAZAMIENTO_X  3200
...
void setup() {
  ...
  stepper.move(DESPLAZAMIENTO_X);
  encoder_ajustado = false;
}

void loop()
{
  if (!encoder_ajustado) ajusta_encoder();
  //read the encoder
  long ct = encoder.count();
  int angulo = AS5047D_Read( AS5047D_select_pin, ANGLECOM) & 0x3FFF;
  int grados360 = angulo / 45.511;
  currentMillis = millis();
  if ((currentMillis - previousMillis >= interval) && (encoder_ajustado)) {
  ...
  if (grados360 < 5) stepper.move(DESPLAZAMIENTO_X);
  if (grados360 > 355) stepper.move(-DESPLAZAMIENTO_X);
  stepper.run();
}
...
{
  int angulo = AS5047D_Read( AS5047D_select_pin, ANGLECOM) & 0x3FFF;
  int grados = (angulo - 8192) / 45.511;
  int grados360 = angulo / 45.511;
  stepper.setMaxSpeed(STEPPER_SPEED * 10);
  stepper.moveTo(-DESPLAZAMIENTO_X * 2);
  stepper.run();
  currentMillis = millis();
  if ((currentMillis - previousMillis >= interval) && (encoder_ajustado)) {
    // save the last time you blinked the LED
